...
function askBot() {
  ...
  $("#send_button").click(function () {
    ...
    if (user_bot_input_text != "") {
      ...
      $("#chat_messages").append(
        '<div class="user__messages">' + user_bot_input_text + " </div>"
      );
      ...
      let chat_input_data = {
        user_message: user_bot_input_text,
      };

      $.ajax({
        type: "POST",

        // Especifique a URL adequada onde você processa as mensagens no seu servidor
        url: "/chat", // Por exemplo, se for "/chat", você deve definir isso em app.py
        ...
        success: function (result) {
          $("#chat-messages").append(
            '<div class="bot__messages">' + result.bot_response + " </div>"
          );
  ...
  // Permite o envio ao pressionar a tecla Enter na caixa de texto
  $("#bot_input_text").keypress(function (e) {
    ...
    if (e.which == 13) {
      ...
      e.preventDefault(); // Evita que a tecla Enter envie um formulário (comportamento padrão)
    }
  });
}
